...
template<typename GENERATOR = QualityXorshift64> struct Random
{
    ...
    int poisson(double lambda)
    {
        assert(lambda > 0);
        int result = -1;
        for(double p = 1; p > exp(-lambda); p *= uniform01()) ++result;
        return result;
    }
    double uniform(double a, double b){return a + (b - a) * uniform01();}
    double exponential01(){return -log(uniform01());}
    static double PI(){return 3.1415926535897932384626433832795;}
    double cauchy01(){return tan((uniform01() - 0.5) * PI());}
    double weibull1(double b){return pow(exponential01(), 1/b);}
    double normal01()
    {
        for(;;)
        {
...
public:
    ...
    int find(ITEM value)
    {//boundary goes to index with smaller cumulative
