...
Vector<unsigned char> RLECompress(Vector<unsigned char> const& byteArray)
{
    ...
    for(int i = 0; i < byteArray.getSize();)
    {
        ...
        int count = 0;
        while(count < 253 && i + count < byteArray.getSize() &&
...
void LZWUncompress(BitStream& in, int maxBits, BitStream& out)
{
    ...
    Vector<Vector<unsigned char> > dictionary(size, size);
...
struct HuffmanTree
{
    ...
    struct Node
    {
        ...
        void append(Bitset<unsigned char>& result)
        {
            ...
            else result.appendValue(letter, 8);
        }
    }* root;
    ...
    Node* readHuffmanTree(BitStream& text)
    {
        Node *left = 0, *right = 0;
        unsigned char letter;
        if(text.readBit()) letter = text.readValue(8);
        else
        {
...
Vector<unsigned char> HuffmanUncompress(Vector<unsigned char> const&
    byteArray)
{
    ...
    Vector<unsigned char> const& byteArray)
{
    ...
    Vector<int> BTWArray = suffixArray(byteArray.getArray(), size,
        BWTRank());
    Vector<unsigned char> result;
...
Vector<unsigned char> BurrowsWheelerReverseTransform(
     Vector<unsigned char> const& byteArray)
{
    int counts[256], firstPositions[256], textSize = byteArray.getSize() - 4;
    for(int i = 0; i < 256; ++i) counts[i] = 0;
    Vector<int> ranks(textSize, textSize);
    for(int i = 0; i < textSize; ++i) ranks[i] = counts[byteArray[i]]++;
    firstPositions[0] = 0;
    for(int i = 0; i < 255; ++i)
        firstPositions[i + 1] = firstPositions[i] + counts[i];
    Vector<unsigned char> index, result(textSize, textSize);
