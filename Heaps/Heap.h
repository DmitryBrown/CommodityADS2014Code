...
public:
    COMPARATOR c;
    Vector<ITEM> items;
    Heap(COMPARATOR const& theComparator = COMPARATOR(),
        REPORTER const& theReporter = REPORTER()): r(theReporter),
        c(theComparator){}
    ...
    typename Handle = int> class IndexedHeap
{
    ...
    struct Reporter{void operator()(Item& item, int i)
        {item.value->value = i;}};
...
public:
    ...
    {
        if(handle >= map.getSize()) for(int i = map.getSize();
            i <= handle; ++i) map.append(-1);
