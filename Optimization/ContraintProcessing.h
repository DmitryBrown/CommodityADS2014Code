...
template<typename CONSTRAINT> struct ConstraintGraph
{
        ...
        CONSTRAINT const& constraint)
    {
        for(int i = 0; i < variables[otherVariable].getSize(); ++i)
            if(variables[otherVariable][i] && constraint.isAllowed(
                variable, value, otherVariable, i)) return true;
        return false;
    }
    ...
    {
        Queue<int> q;
        Vector<bool> onQ(g.nVertices(), g.nVertices(), true);
...
struct AllDifferent
{
    ...
    {
        LinearProbingHashTable<int, bool>& variables;
        bool isAllowed(int variable, int value, int variable2,
            int value2)const
        {
