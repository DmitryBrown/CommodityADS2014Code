...
template<typename EDGE_DATA> class GraphAA
{
    struct Edge
    {
        int to;
        EDGE_DATA edgeData;
        Edge(int theTo, EDGE_DATA const& theEdgeData):
            to(theTo), edgeData(theEdgeData){}
    };
    Vector<Vector<Edge> > vertices;
public:
    GraphAA(int initialSize = 0): vertices(initialSize, initialSize){}
...
template<typename GRAPH, typename ACTION> void DFS(GRAPH const& g,
    ACTION& a = ACTION())
{
    Vector<bool> visited(g.nVertices(), g.nVertices(), false);
...
{
    Vector<int> distances(g.nVertices(), g.nVertices(), -1);
...
{
    Vector<int> mst(g.nVertices(), g.nVertices(), -1);
...
Vector<int> ShortestPath(GRAPH& g, int from, int dest = -1)
{
    ...
    Vector<int> pred(g.nVertices(), g.nVertices(), -1);
    typedef KVPair<double, int> QNode;
    IndexedArrayHeap<QNode, KVComparator<double, int> > pQ;
...
template<typename GRAPH> struct BellmanFord
{
    ...
    BellmanFord(GRAPH& g, int from): v(g.nVertices()), pred(v, v, -1),
    distances(v, v, numeric_limits<double>::max()), hasNegativeCycle(false)
    {
        assert(from >= 0 && from < v);
        Vector<bool> onQ(v, v, 0);
        Queue<int> queue;
        int cost = 0;
        distances[from] = 0;
        onQ[from] = true;
...
template<typename GRAPH> class ShortestAugmentingPath
{
    ...
    bool hasMinCostAugmentingPath(GRAPH& g,
        Vector<FlowData>& fedges, int from, int to)
    {
...
public:
    ...
    ShortestAugmentingPath(GRAPH& g, Vector<FlowData>& fedges, int from,
        ...
        path(v, v, -1), pred(v, v, -1), neededFlow(theNeededFlow)
    {
...
{//v = n + m + 2, e = n + m + allowedMatches.getSize()
    ...
    for(int i = 0; i < allowedMatches.getSize(); ++i)
    {
        data.append(FlowData(allowedMatches[i].first.first,
            1, allowedMatches[i].second));
    ...
    Vector<Vector<int> > const& menScores)
{
    ...
    Vector<int> currentMan(m, m, -1), nextWoman(n, n, 0);
