...
struct FairHash
{
    ...
    unsigned int hash(unsigned char* array, int size)
    {
...
struct LCGHash
{
    unsigned int hash(unsigned int x){return 1099087573u * x;}
    template<typename NUMBER> unsigned int hash(NUMBER* array, int size)
    {
        unsigned int sum = 0;
...
struct FNVHash64
{
    template<typename POD> unsigned long long hash(POD const& x)
        {return hash((unsigned char*)&x, sizeof(x));}
    unsigned long long hash(unsigned char* array, int size)
    {
...
public:
    ...
    unsigned int hash(unsigned int x){return Xorshift::transform(seed + x);}
    template<typename NUMBER> unsigned int hash(NUMBER* array, int size)
    {
...
public:
    ...
    unsigned int hash(unsigned int x){return seed * x % PRIME;}
    template<typename NUMBER> unsigned int hash(NUMBER* array, int size)
...
template<typename HASHER = PrimeHash> struct EHash32
{
    ...
    WORD hash(WORD x){return h.hash(x);}
    WORD hash(int x){return h.hash((WORD)x);}
    WORD hash(short x){return h.hash((WORD)x);}
    WORD hash(unsigned short x){return h.hash((WORD)x);}
    template<typename POD> WORD hash(POD x)
    {
...
template<typename HASHER = PrimeHash> struct EHash64
{
    ...
    WORD hash(WORD x){return h.hash(x);}
    WORD hash(long long x){return h.hash((WORD)x);}
    WORD hash(int unsigned x){return h.hash((WORD)x);}
    WORD hash(int x){return h.hash((WORD)x);}
    WORD hash(short x){return h.hash((WORD)x);}
    WORD hash(unsigned short x){return h.hash((WORD)x);}
    template<typename POD> WORD hash(POD x)
...
public:
    ...
    unsigned int update(unsigned int currentHash, unsigned char byte)
        {return currentHash ^ table[byte];}//for both additions and removals
};
...
public:
    ...
    template<typename POD> unsigned int hash(POD* array, int size)
...
public:
    ...
    unsigned int hash(unsigned int x){return (a * x) >> wLB;}
    template<typename POD> unsigned int hash(POD* array, int size)
        {return h.hash(array, size);}
};
...
{
    HASHER h;
    unsigned int hash(string const& item)
        {return h.hash(item.c_str(), item.size());}
    template<typename VECTOR> unsigned int hash(VECTOR const& item)
