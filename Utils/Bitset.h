...
public:
    Bitset(unsigned long long initialSize = 0):
       bitSize(initialSize), storage(wordsNeeded(), wordsNeeded(), 0){}
    ...
    {
        assert(n <= numeric_limits<unsigned long long>::digits
            && i >= 0 && i + n <= bitSize);
    ...
    unsigned long long getBitReversedValue(int i, int n)const
    {
        ...
        //xxxxyyyy|yyyyyyyy|yyxxxxxx, y are the affected bits
    ...
    int popCount()
    {
        zeroOutRemainder();
        int sum = 0;
        for(int i = 0; i < wordSize(); ++i) sum += popCount(storage[i]);
        return sum;
    }
};

template<int N, typename WORD = unsigned long long> class NBitVector
{
    Bitset<WORD> bitset;
public:
