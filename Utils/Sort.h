...
template<typename ITEM, typename COMPARATOR>
void mergeSort(ITEM* vector, int left, int right, COMPARATOR const& c)
{
    int size = right - left + 1;
    if(size < 2) return;
    ITEM* storage = rawMemory<ITEM>(size);
    for(int k = left; k <= right; ++k) new(&storage[k])ITEM(vector[k]);
    mergeSortHelper(vector, left, right, c, storage);
    rawDestruct(storage, size);
}
template<typename ITEM> void mergeSort(ITEM* vector, int left, int right)
    {mergeSort(vector, left, right, DefaultComparator<ITEM>());}
...
void countingSort(int* vector, int n, int max)
{
...
template<typename ITEM, typename ORDERED_HASH> void KSort(ITEM* a, int n,
    int max, ORDERED_HASH const& h)
{
    ITEM* temp = rawMemory<ITEM>(n);
    Vector<int> count(max + 1, max + 1, 0);
    for(int i = 0; i < n; ++i) ++count[h(a[i]) + 1];
    for(int i = 0; i < max; ++i) count[i + 1] += count[i];//accumulate counts
