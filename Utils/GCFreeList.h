...
template<typename ITEM> struct SimpleDoublyLinkedList
{
    struct Node
    {
        ...
        Node(ARGUMENT const& argument, Node* theNext):
            item(argument), next(theNext), prev(0) {}
    }*root;
    SimpleDoublyLinkedList(): root(0){}
...
public:
    ...
    ITEM* allocate()
    {
        ...
        if(root->item.isFull()){next.cut(root); full.prepend(root);}
        return (ITEM*)result;
    }
    void remove(ITEM* item)
    {
