...
public:
    HashQ(VECTOR const& theText, int theTextSize, VECTOR const& thePattern,
        ...
        shift(patternSize/q, patternSize/q), h(theHash)
    {
...
public:
    WuManber(unsigned char* theText, int theTextSize,
        Vector<pair<unsigned char*, int> >& thePatterns, HASH theHash =
        HASH()): text(theText), patterns(thePatterns), position(0),
        h(theHash), q(3), textSize(theTextSize),
        minPatternSize(numeric_limits<int>::max())
    {
        ...
        for(int j = 0; j < patterns.getSize(); ++j)
            ...
            {
                int l = h(patterns[j].first, i);
                if(i == temp) candidates[l].append(j);
                else shift[l] = min(temp - i, shift[l]);
            }
    }
    int findNext(Vector<int>& matches)
    {
        ...
        {
            int l = h(text, position + minPatternSize - q),
                result = position;
            findMatches(l, matches);
            position += shift[l];
...
struct RegularExpressionMatcher
{
    ...
    int m;
    GraphAA<bool> g;
    RegularExpressionMatcher(string const& theRe):
        re(theRe), m(re.length()), g(m + 1)
    {
        Stack<int> clauses;
        for(int i = 0; i < m; ++i)
        {
    ...
    {
        Vector<bool> visited;
        VisitedAction(int nVertices): visited(nVertices, nVertices, false){}
        void treeEdge(int v){}
...
public:
    ShiftAnd(unsigned char* theText, int theTextSize, unsigned char*
        thePattern, int thePatternSize): position(0), textSize(theTextSize),
        text(theText), patternSize(thePatternSize), pattern(thePattern),
        state(0)
    {
...
Vector<Edit> DiffInternal(Vector<CHAR>const& a, Vector<CHAR>const& b)
{
    ...
    Vector<int> frontierX(size, size, -2);
    Vector<Edit*> edits(size, size, 0);
    ...
    Vector<Edit> result;
    for(Edit* link = edits[d]; link; link = link->prev)
        result.append(*link);
    result.reverse();
